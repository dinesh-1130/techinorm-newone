...
export default function ScrollSections() {
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e) => {
      // If we're on the last section and body overflow is auto, allow normal scrolling
      if (
      ...
      // Allow natural scrolling on mobile - don't prevent default
      ...
      // Only prevent default if we're about to trigger a section change
      if (Math.abs(deltaY) > threshold) {
        const isLast = currentIndex === sections.length - 1;
        const isFirst = currentIndex === 0;

        if ((deltaY > 0 && !isLast) || (deltaY < 0 && !isFirst)) {
          e.preventDefault();
        }
      }
    };
    ...
    const handleTouchEnd = (e) => {
      ...
      if (
        ...
        return; // Don't handle touch end, allow normal scrolling
      }
  ...
  const handleContainerClick = (e) => {
    // Don't handle clicks if normal scrolling is enabled (after Web Development)
    if (
      currentIndex === sections.length - 1 &&
      document.body.style.overflow === "auto"
    ) {
    ...
    // Only handle clicks on mobile and when not already scrolling
    if (scrolling || window.innerWidth >= 768) return;
